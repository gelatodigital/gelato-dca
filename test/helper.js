...
module.exports.assertMost = function (val1, val2, errorStr) {
  assert(
    new BN(val1).should.be.a.bignumber.that.is.most(new BN(val2)),
    errorStr,
  );
};
...
module.exports.calcDstQty = function (srcQty, srcDecimals, dstDecimals, rate) {
  ...
  if (dstDecimals.gte(srcDecimals)) {
    ...
    result = srcQty
      ...
      .div(
        precisionUnits.mul(
          new BN(10).pow(new BN(srcDecimals.sub(dstDecimals))),
        ),
      );
  }
  return result;
...
module.exports.zeroNetworkBalance = async function (network, tokens, admin) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (balance.gt(zeroBN)) {
      await network.withdrawToken(tokens[i].address, balance, admin, {
        from: admin,
      });
    }
  }
