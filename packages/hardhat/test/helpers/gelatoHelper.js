...
module.exports.getGelatoForExec = async () => {
  const [, , executorWallet] = await ethers.getSigners();
  return await ethers.getContractAt(
    "IGelato",
    (await deployments.get("Gelato")).address,
    executorWallet
  );
};

module.exports.getTxCostsInToken = async (gasUsed, gasPrice, token) => {
  const oracleAggregator = await ethers.getContractAt(
    "IOracleAggregator",
    network.config.OracleAggregator
  );

  const [ethCostInToken] = await oracleAggregator.getExpectedReturnAmount(
    gasUsed.mul(gasPrice),
    network.config.ETH,
    token
  );
  return ethCostInToken;
};

module.exports.getSubmittedTaskV2 = async (serviec) => {
  const block = await ethers.provider.getBlock();
  const topics = serviec.filters.LogTaskSubmitted().topics;
  const filter = {
    address: serviec.address.toLowerCase(),
  ...
  const event = serviec.interface.parseLog(logs[0]);
  return event.args;
};

module.exports.getUpdatedTaskV2 = async (serviec) => {
  const block = await ethers.provider.getBlock();
  const topics = serviec.filters.LogTaskUpdated().topics;
  const filter = {
    address: serviec.address.toLowerCase(),
    blockhash: block.hash,
    topics,
