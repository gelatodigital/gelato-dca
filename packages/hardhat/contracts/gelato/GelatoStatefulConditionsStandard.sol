...
abstract contract GelatoStatefulConditionsStandard is GelatoConditionsStandard {
  ...
  function _getIdOfNextTaskInCycle()
    internal
    view
    returns (uint256 nextTaskReceiptId)
  {
    try gelatoCore.currentTaskReceiptId() returns (uint256 currentId) {
      ...
      revert(
        string(
          abi.encodePacked(
            'GelatoStatefulConditionsStandard._getIdOfNextTaskInCycle',
            _err
          )
        )
      );
      ...
      revert(
        'GelatoStatefulConditionsStandard._getIdOfNextTaskInCycle:undefined'
      );
    }
  }
}
